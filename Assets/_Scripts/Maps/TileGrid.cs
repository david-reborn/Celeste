        ...
        else
        {
            ...
            for (int index1 = 0; index1 < left; ++index1)
            {
                for (int index2 = 0; index2 < rows; ++index2)
                    virtualMap[index1, index2] = this.Tiles[0, Util.Clamp(index2 - up, 0, this.TilesY - 1)];
            }
            for (int index1 = columns - right; index1 < columns; ++index1)
            {
                for (int index2 = 0; index2 < rows; ++index2)
                    virtualMap[index1, index2] = this.Tiles[this.TilesX - 1, Util.Clamp(index2 - up, 0, this.TilesY - 1)];
            }
            for (int index1 = 0; index1 < up; ++index1)
            {
                for (int index2 = 0; index2 < columns; ++index2)
                    virtualMap[index2, index1] = this.Tiles[Util.Clamp(index2 - left, 0, this.TilesX - 1), 0];
            }
            for (int index1 = rows - down; index1 < rows; ++index1)
            {
                for (int index2 = 0; index2 < columns; ++index2)
                    virtualMap[index2, index1] = this.Tiles[Util.Clamp(index2 - left, 0, this.TilesX - 1), this.TilesY - 1];
            }
